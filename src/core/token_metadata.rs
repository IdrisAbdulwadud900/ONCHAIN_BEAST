...
impl TokenMetadataService {
    ...
    pub async fn get_token_metadata_batch(&self, mints: &[String]) -> BeastResult<HashMap<String, TokenMetadata>> {
    ...
    async fn fetch_token_metadata(&self, mint: &str) -> BeastResult<TokenMetadata> {
        ...
        let (symbol, name, logo_uri) = self.fetch_metaplex_metadata(mint).await
            .unwrap_or_else(|_| (
    ...
    fn parse_mint_account(&self, account_info: &serde_json::Value) -> BeastResult<MintAccountData> {
        let data = account_info.get("data")
    ...
    async fn fetch_metaplex_metadata(&self, mint: &str) -> BeastResult<(String, String, Option<String>)> {
        ...
        let account_info = match self.fetch_account_info(&metadata_address).await {
            Ok(info) => info,
            Err(_) => return Err(BeastError::NotFound("Metaplex metadata not found".to_string())),
        };
    ...
    fn find_metadata_address(&self, mint: &str) -> BeastResult<String> {
        ...
        Err(BeastError::NotFound("PDA derivation not implemented".to_string()))
    }
    ...
    pub async fn get_token_account_owner(&self, token_account: &str) -> BeastResult<String> {
        ...
        let owner = account_info.get("data")
            .and_then(|d| d.get("parsed"))
            .and_then(|p| p.get("info"))
            .and_then(|i| i.get("owner"))
            .and_then(|o| o.as_str())
            .ok_or_else(|| BeastError::ParseError("Failed to get token account owner".to_string()))?;
...
mod tests {
    ...
    async fn test_generate_symbol_from_mint() {
        ...
        let symbol = service.generate_symbol_from_mint("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
    ...
    async fn test_cache_preload() {
        ...
        let usdc = service.get_token_metadata("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v").await;
        assert!(usdc.is_ok());
